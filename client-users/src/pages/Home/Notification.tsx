...
function Notification(){
  ...
  async function registerEmail(){
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_PORT}/mailing/subscribe`, {
        method: "POST",
        mode: "cors",
        headers: {
      ...
      <form className = {styles["email-newsletter"]} noValidate>
        ...
        <div className = {styles["form-elem-container"]}>
          <div className = {styles["input-and-button"]}>
            ...
            }} placeholder= "Enter your email" type = "email" name = "email" id = "email" autoComplete='off'/>
            <button onClick = {
              async (e) => {
                e.preventDefault();
                await registerEmail();
          ...
          {subscribed && <span className = {`${styles.message} success`}>You have successfully subscribed.</span>}
          {hasInvalidEmail && <span className = {`${styles.message} error`}>Your email is invalid. Please try again.</span>}
        </div>
      </form>
    </section>
  )
