...
const SignInForm : FC<FormProps> = ({getSignedInStatus}) => {
  ...
  function setErrors(data : dataErrors){
    for(const key in data){
      setFormErrors(prevFormErrors => ({...prevFormErrors, [key]: data[key].msg}));
    }
  }
  ...
  return(
    <div className = {styles["form-container"]}>
      <form className = {styles["sign-in-form"]} method="POST" action = "/login" onSubmit = {(e) => handlePost(e)} noValidate>
        ...
        <div className= {styles["password-button-container"]}>
          {!passwordVisible ? 
            ...
            :
            <>
              <input type = "text" id= "password" name = "password" autoComplete="off" onChange = {(e) => changePasswordInput(e)} value = {passwordInput} required/>
              <button type = "button" onClick = {togglePasswordVisible}>    
